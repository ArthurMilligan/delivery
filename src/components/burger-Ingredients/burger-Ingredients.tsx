...
const BurgerIngredients: FC = (props) => {
  ...
  const handleScroll = (e: UIEvent<HTMLDivElement>) => {
    ...
    tmp.forEach((i, number) => {
      if (
        ...
        Math.abs(i.current.getBoundingClientRect().y - switchPanelCheckPoint.current.getBoundingClientRect().y) < flag
      ) {
        current = `${number}`;
  ...
  return (
    <section className={style.section}>
      ...
      <div className='mb-10'>
        <SwitchPanel currentSwitchPanel={currentSwitchPanel} setCurrentSwitchPanel={setCurrentSwitchPanel} />
      </div>
      <div className={style.sectionList} onScroll={handleScroll} ref={switchPanelCheckPoint}>
        ...
        <div className={`${style.sectionListItems} pl-4`}>
          {items.map((i) => {
            if (i.type === 'bun') return <ListElement id={i._id} key={i._id} img={i.image} price={i.price} name={i.name} />;
            else return null;
          })}
        </div>
        ...
        <div className={`${style.sectionListItems} pl-4`}>
          {items.map((i) => {
            if (i.type === 'sauce') return <ListElement id={i._id} key={i._id} img={i.image} price={i.price} name={i.name} />;
            else return null;
          })}
        </div>
        ...
        <div className={`${style.sectionListItems} pl-4`}>
          {items.map((i) => {
            if (i.type === 'main') return <ListElement id={i._id} key={i._id} img={i.image} price={i.price} name={i.name} />;
            else return null;
          })}
        </div>
      </div>
